...
public class CopyHandler implements ExistingEntityHandler {
    ...
    public void processExistingResource(HttpManager manager, Request request, Response response, Resource resource) throws NotAuthorizedException, BadRequestException, ConflictException {
        ...
        if (rDest == null) {
        ...
        } else if (!(rDest instanceof CollectionResource colDest)) {
            ...
            if (handlerHelper.isLockedOut(request, fDest)) {
                ...
                if (rExisting != null) {
                    ...
                    if (Objects.equals(sourceUniqueId, destinationUniqueId)) {
                        // http://www.webdav.org/specs/rfc4918.html#rfc.section.9.8.5
                        log.info("the source and destination resources are the same resource. source unique id: {} dest unique id: {}", sourceUniqueId, destinationUniqueId);
                    ...
                    if (!canOverwrite(request)) {
                        ...
                        log.info("destination resource exists, and overwrite header is not set. dest name: {} dest folder: {}", dest.name, colDest.getName());
                        ...
                        if (deleteHelper.isLockedOut(request, rExisting)) {
                            log.info("destination resource exists, and overwrite header IS set, but destination is locked. dest name: {} dest folder: {}", dest.name, colDest.getName());
                            responseHandler.respondPreconditionFailed(request, response, resource);
                            return;
                        } else {
                            if (deleteExistingBeforeCopy) {
                                if (rExisting instanceof DeletableResource dr) {
